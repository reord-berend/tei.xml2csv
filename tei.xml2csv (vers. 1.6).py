...
if find_dates is not None:
    ...
    # Text in this field is sometimes separated by new lines, carriage returns, or repeated tabs, which must be removed
    if '\n' in dates or '\r' in dates or '\t' in dates:
        dates = dates.replace('\n', '')
        dates = dates.replace('\r', '')
        dates = dates.replace('\t', '')
        # Sometimes there are also repeated spaces between words to be removed
        for text in dates:
            dates = dates.replace('  ', ' ')

...
# Defining a Python class for TEI-XML files, with SDBM fields as properties
class TEIFile(object):
    ...
    def source_catalog_or_lot_number(self):
        self._source_catalog_or_lot_number = self.elem_to_text(self.soup.idno)
        return self._source_catalog_or_lot_number
    
    @property
    # Find source institution data (pulling values from two different elements)
    def institution(self):
        find_repository = self.elem_to_text(self.soup.repository)
        find_institution = self.elem_to_text(self.soup.institution)
    ...
    def titles(self):
        if self.soup.mscontents:
            if self.soup.mscontents.title:
                titles = self.elem_to_text(self.soup.mscontents.title)
            elif self.soup.msdesc.title:
                titles = self.elem_to_text(self.soup.msdesc.title)
            else:
                titles = self.elem_to_text(self.soup.title)
        elif self.soup.msdesc.title:
            titles = self.elem_to_text(self.soup.msdesc.title)
        else:
            titles = self.elem_to_text(self.soup.title)
    ...
    def authors(self):
        self._authors = self.elem_to_text(self.soup.author)
        return self._authors
    
    @property
    # To find manuscript dates
    def dates(self):
        find_dates = self.elem_to_text(self.soup.origdate)
    ...
    def languages(self):
        languages = self.elem_to_text(self.soup.textlang)
    ...
    def materials(self):
        ...
        materials = ''
        if self.soup.supportdesc:
            if self.soup.supportdesc.support:
                if self.soup.supportdesc.support.p:
    ...
    def width(self):
        width = self.elem_to_text(self.soup.width)
        # The unit of measurement may be found as attributes of several elements
    ...
    def binding(self):
        ...
        self._binding = self.elem_to_text(binding_desc)
...
for tei_file in files:
    try:
        entries = tei_to_csv_entry(tei_file)
    except Exception as ex:
        print(f"Error processing '{tei_file}': {ex}")
...
with open('C:\\Users\\...', 'w', encoding='utf8', newline='') as f:
    # identify headers 
    header = ['id', 'manuscript','groups', 'source_date', 'source_title', 'source_catalog_or_lot_number', 'institution', 'sale_selling_agent', 'sale_seller_or_holder', 'sale_buyer', 'sale_sold', 'sale_ price', 'titles', 'authors', 'dates', 'artists', 'scribes', 'languages', 'materials', 'places', 'uses', 'folios', 'num_columns', 'num_lines', 'height', 'width', 'alt_size', 'miniatures_fullpage', 'miniatures_large', 'miniatures_small', 'miniatures_unspec_size', 'initials_historiated', 'initials_decorated', 'manuscript_binding', 'manuscript_link', 'other_info', 'provenance']
    writer = csv.writer(f)
